...
function Get-ValidRTSPPaths {
    ...
    # This list combines common vendor paths for almost all known IP camera brands.
    $Paths = @( 
        # Hikvision & Clones
        "Streaming/Channels/101", "Streaming/Channels/102", "Streaming/Channels/1", "Streaming/Channels/2",
        "h264/ch1/main/av_stream", "h264/ch1/sub/av_stream", "ch01/0", "Streaming/Unicast/channels/101",
        
        # Dahua, Amcrest, Lorex
        "cam/realmonitor?channel=1&subtype=0", "cam/realmonitor?channel=1&subtype=1", 
        "cam/realmonitor?channel=0&subtype=0", "cam/realmonitor?channel=1&subtype=0&authbasic=64",
        
        # Axis
        "axis-media/media.amp", "axis-media/media.amp?camera=1", "axis-media/media.amp?videocodec=h264",
        "axis-media/media.3gp", "rtsp_tunnel",
        
        # Reolink
        "h264Preview_01_main", "h264Preview_01_sub", "h265Preview_01_main", "preview",
        
        # Sony & Panasonic
        "media/video1", "media/video2", "media/video3", "MediaInput/h264", "MediaInput/h264/stream_1",
        "MediaInput/h264/stream_2", "video1", "video2",
        
        # TP-Link (Tapo/Vigi) & Foscam
        "stream1", "stream2", "videoMain", "videoSub", "live1.sdp", "live2.sdp",
        
        # Hanwha / Samsung / Wisenet
        "profile1/media.smp", "profile2/media.smp", "profile3/media.smp", "snwms/live/ch0", "LiveChannel/0/media.smp",
        
        # Bosch & Pelco
        "rtsp_tunnel?h264=1", "rtsp_tunnel?h265=1", "line1", "line2", "stream",
        
        # XMeye / Xiongmai (Common Cheap OEM)
        "live0.264", "live1.264", "user=admin&password=&channel=1&stream=0.sdp", "live/main",
        
        # Vivotek & Mobotix
        "live.sdp", "video.h264", "video.h265", "control/faststream.jpg?stream=full",
        
        # Generic ONVIF & Miscellaneous
        "1", "0/1", "1/1", "0", "11", "12", "ch0", "ch1", "video", "av0_0", "live", "live.sdp", "onvif1", "onvif2",
        "h264", "h264.sdp", "mpeg4", "mpeg4.sdp", "media.smp", "video.mp4", "img/video.sav", "img/media.sav"
    )

    # --- FAST SCAN "COMMON" PATHS ---
    # These represent the "Heavy Hitters" - the most likely paths to succeed across all major brands.
    $CommonPaths = @( 
        ...
        "live0.264",                           # XMeye / Generic Chinese OEM
        "media/video1",                        # Sony / Panasonic
        "videoMain",                           # Foscam
        "live.sdp",                            # Vivotek / Generic
        "rtsp_tunnel",                         # Bosch / Axis Tunnel
        "onvif1",                              # Standard ONVIF
        "1",                                   # Generic Shortpath
        "ch01/0"                               # Older Hikvision/Generic
    )
    ...
    if ($UseCommonPaths) {
        Write-Host "Using common paths only (High-Speed Multi-Vendor Mode)`r`n" -ForegroundColor Cyan
    } else {
        Write-Host "Running Deep Path Scan (Comprehensive Mode)`r`n" -ForegroundColor Magenta
    }
    ...
    foreach ($portGroup in $portsByIPPort) {
        $portIndex++
        $openPort = $portGroup.Group[0]
        $ip = $openPort.IPAddress
        $port = $openPort.Port
        $authType = $openPort.authType

        Write-Host -NoNewline "Scanning $ip`:$port (Target $portIndex/$totalPorts)... "
        ...
        foreach ($path in $pathsToScan) {
            try {
                $pathIndex++
                if ($pathIndex % 10 -eq 0 -or $pathIndex -eq $totalPaths) {
                    Write-Host -NoNewline "`rScanning $ip`:$port (Target $portIndex/$totalPorts) - Trying Path $pathIndex/$totalPaths... "
                }
                ...
                if ($response.Success) {
                    if ($response.StatusCode -eq 401 -or $response.StatusCode -eq 403) {
                        ...
                        $foundAuthPath = $true
                        # In common mode, we stop at the first valid path to save time
                        if ($UseCommonPaths) { break }
                    }
                }
            }
            catch [System.OperationCanceledException] {
                Write-Host "`r`n[!] Scan interrupted by user." -ForegroundColor Yellow
                break
            }
            catch { continue }
        }
        ...
        if ($foundAuthPath) {
            Write-Host "`rScanning $ip`:$port complete - Found valid stream path.                    " -ForegroundColor Green
        } else {
            Write-Host "`rScanning $ip`:$port complete - No valid paths found.                    " -ForegroundColor Red
        }
    }
